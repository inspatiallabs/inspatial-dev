 ...
 */
import { EventSystemStatsType, PlatformType, TriggerEventDeliveryStatusType } from "./types.ts";
import { errorLogger, LogSeverityEnum, ErrorCodeEnum } from "./errors.ts";
...
export class TriggerPerformanceMonitorClass {
  ...
  private eventTimers: Map<string, { 
    start: number, 
    queueStart?: number,
    handlerStart?: number
  }> = new Map();
  ...
  public static getInstance(): TriggerPerformanceMonitorClass {
    if (!TriggerPerformanceMonitorClass.instance) {
      TriggerPerformanceMonitorClass.instance = new TriggerPerformanceMonitorClass();
    }
  ...
  public trackEventCompletion(
    ...
    this.stats.averageProcessingTime = this.totalProcessingTime / this.stats.totalEventsProcessed;
  ...
  public getSystemHealth(): TriggerSystemHealthType {
    ...
    const avgQueueSize = this.queueSizeBuffer.length > 0
      ? this.queueSizeBuffer.reduce((sum, size) => sum + size, 0) / this.queueSizeBuffer.length
      : 0;
    
    const queueHealthPercent = Math.min(100, (avgQueueSize / this.stats.maxQueueSize) * 100);
    const queueHealth = 
      queueHealthPercent > 80 ? "critical" :
      queueHealthPercent > 50 ? "warning" : "good";
    
    // Error rate based on recent errors
    const errorRate = this.errorBuffer.length > 0
      ? this.errorBuffer.filter(isError => isError).length / this.errorBuffer.length
      : 0;
    ...
    const avgLatency = this.processingTimeBuffer.length > 0
      ? this.processingTimeBuffer.reduce((sum, time) => sum + time, 0) / this.processingTimeBuffer.length
      : 0;
    ...
    const latencyHealth = 
      avgLatency > latencyThreshold * 1.5 ? "critical" :
      avgLatency > latencyThreshold * 0.8 ? "warning" : "good";
    
    // Overall health is the worst of all indicators
    const healthValues = [queueHealth, errorHealth, latencyHealth];
    const overall = 
      healthValues.includes("critical") ? "critical" :
      healthValues.includes("warning") ? "warning" : "good";
 ...
 * Singleton instance
 */
export const triggerPerformanceMonitor = TriggerPerformanceMonitorClass.getInstance(); 