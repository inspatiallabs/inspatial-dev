/**
 * Fixed test for the ParentNode implementation issue
 */
...
const createParentWithChildren = (doc: Document, childElements = 0, childTextNodes = 0, childComments = 0) => {
  ...
  const parent = new TestDocumentFragment(doc);
...
/**
 * Test suite for the ParentNode class using our custom TestDocumentFragment
 */
test({
  name: "Fixed: ParentNode.children returns only Element nodes",
  fn: () => {
...
test({
  name: "Fixed: ParentNode.children returns empty NodeList when no element children exist",
  fn: () => {
    const doc = createTestDocument();
...
test({
  name: "Fixed: ParentNode.children updates when elements are added or removed",
  fn: () => {
    const doc = createTestDocument();
...
test({
  name: "Fixed: ParentNode.children ignores text nodes added between elements",
  fn: () => {
    const doc = createTestDocument();
    const parent = createParentWithChildren(doc, 2, 0, 0);
...
test({
  name: "Fixed: ParentNode.childElementCount matches children.length",
  fn: () => {
    const doc = createTestDocument();
