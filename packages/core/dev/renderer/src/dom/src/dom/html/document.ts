...
const createHTMLElement = (ownerDocument: any, builtin: boolean, localName: string, options: any): HTMLElement => {
...
export class HTMLDocument extends Document {
  ...
  get head(): HTMLElement {
    ...
    let firstElementChild = documentElement.firstElementChild as HTMLElement | null;
  ...
  override createElement(localName: string, options?: { is?: string }): HTMLElement {
    const builtin = !!(options && options.is);
    const element = createHTMLElement(this, builtin, localName, options);
    if (builtin && options?.is)
