...
import { getMockCalls, mockSession, spy } from "../../test/src/mock/mock.ts";
...
test("should run effect on change", () => {
  ...
  setX(20);
  if (effectInstance) {
    (effectInstance as any)._notify(STATE_DIRTY);
    (effectInstance as any)._runEffect();
  }
...
test("should handle nested effect", () => {
  ...
  const innerPureDispose = spy();
  const innerEffectDispose = spy();
  ...
  if (innerEffectInstance) {
    ...
    (innerEffectInstance as any)._notify(STATE_DIRTY);
    console.log("Running inner effect after Y change");
    (innerEffectInstance as any)._runEffect();
  }
  ...
  expect(getMockCalls(innerPureDispose).length).toBe(
    initialPureDisposeCalls + 1
  ); // One cleanup
  expect(getMockCalls(innerEffectDispose).length).toBe(
    initialEffectDisposeCalls + 1
  ); // One disposer
  ...
  const beforeSecondYEffectDisposeCalls =
    getMockCalls(innerEffectDispose).length;
  ...
  expect(getMockCalls(innerPureDispose).length).toBe(
    beforeSecondYPureDisposeCalls + 1
  ); // One more cleanup
  expect(getMockCalls(innerEffectDispose).length).toBe(
    beforeSecondYEffectDisposeCalls + 1
  ); // One more disposer
  ...
  if (outerEffectInstance) {
    ...
    (outerEffectInstance as any)._notify(STATE_DIRTY);
    (outerEffectInstance as any)._runEffect();
  }
  ...
  expect(getMockCalls(innerPureDispose).length).toBe(
    beforeXPureDisposeCalls + 1
  );
  expect(getMockCalls(innerEffectDispose).length).toBe(
    beforeXEffectDisposeCalls + 1
  );
  ...
  expect(getMockCalls(innerPureDispose).length).toBe(
    beforeY3PureDisposeCalls + 1
  );
  expect(getMockCalls(innerEffectDispose).length).toBe(
    beforeY3EffectDisposeCalls + 1
  );
  ...
  expect(getMockCalls(innerPureDispose).length).toBe(
    beforeStopPureDisposeCalls + 1
  ); // One more cleanup
  expect(getMockCalls(innerEffectDispose).length).toBe(
    beforeStopEffectDisposeCalls + 1
  ); // One more disposer
