...
describe("merge", () => {
  ...
  it("overrides enumerables", () => {
    ...
    expect(
      Object.getOwnPropertyDescriptor(props, "value1")?.enumerable
    ).toBeTruthy();
  ...
  it("fails with non objects sources", () => {
    expect(() => merge({ value: 1 }, true as any)).toThrow();
    expect(() => merge({ value: 1 }, 1 as any)).toThrow();
  });
