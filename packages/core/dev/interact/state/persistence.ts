...
/**
 * InCloud storage adapter implementation (placeholder for future integration)
 *
 * This is a placeholder for the upcoming InCloud integration.
 * The actual implementation will be added in a future release.
 */
const inCloudAdapter = (config: any = {}): StorageAdapterType => {
  ...
  return {
    async getItem(key: string): Promise<string | null> {
      console.log(`[InCloud Placeholder] getItem called for key: ${key}`);
      console.log(
    ...
    async setItem(key: string, value: string): Promise<void> {
      console.log(`[InCloud Placeholder] setItem called for key: ${key}`);
      console.log(
        "InCloud integration coming soon - using memory storage for now"
      );
    ...
    async removeItem(key: string): Promise<void> {
      console.log(`[InCloud Placeholder] removeItem called for key: ${key}`);
      console.log(
        "InCloud integration coming soon - using memory storage for now"
      );
    ...
    async clear(): Promise<void> {
      console.log(`[InCloud Placeholder] clear called`);
      console.log(
        "InCloud integration coming soon - using memory storage for now"
      );
...
registerStorageAdapter("inCloud", inCloudAdapter());
...
export function setupPersistence<T extends object>(
  ...
  const config: PersistOptionsType =
    options === true
      ? {
  ...
  if (typeof config.storage === "string") {
    ...
    if (!adapter) {
      console.error(
        `Storage adapter "${config.storage}" not found. Using memory adapter.`
      );
  ...
  const unsubscribe =
    config.autoSave !== false ? state.subscribe(debouncedSave) : () => {};
...
export async function clearAllPersistedState(
  ...
  if (adapter.clear) {
    ...
    console.error(
      `Storage adapter "${storageType}" does not support clearing all state.`
    );
  }
}

/**
 * Setup InCloud persistence for a state instance
 *
 * This is a convenience function that configures persistence using the InCloud adapter.
 * Currently uses a placeholder implementation that will be replaced in a future version.
 *
 * @param state The state instance to persist
 * @param options InCloud-specific configuration options
 * @returns Cleanup function to disable persistence
 */
export function setupInCloudPersistence<T extends object>(
  state: StateInstanceType<T>,
  options: {
    key?: string;
    autoSave?: boolean;
  ...
  // Set up standard persistence with InCloud adapter
