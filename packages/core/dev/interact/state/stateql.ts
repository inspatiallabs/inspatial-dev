...
import type { StateInstanceType, StateQLInstanceType } from "./types.ts";
...
function parseConditional(statement: string, values: any[]): Operation | null {
  ...
  const conditionMatch = statement.match(
    /if\s*\((.*?)\)\s*\{(.*?)\}(?:\s*else\s*\{(.*?)\})?/s
  );
  ...
  return {
    ...
    value: null,
    condition: {
      test: testFn as (state: any) => boolean,
      thenOps,
...
function parseOperation(statement: string, values: any[]): Operation | null {
  ...
  const arrayOpMatch = statement.match(
    /^\s*([\w.[\]]+)\.(push|pop|shift|unshift|splice)\((.*)\)\s*$/
  );
  if (arrayOpMatch) {
    const [, path, method, args] = arrayOpMatch;
    const parsedArgs = args
      ? args.split(",").map((arg) => parseValue(arg.trim(), values))
      : [];
 ...
 */
function applyOperations<T extends object>(
  state: T,
  ...
  for (const op of operations) {
    ...
    switch (op.operator) {
      ...
      case OperationType.SPLICE:
        if (Array.isArray(target[prop])) {
          const [start, deleteCount, ...items] = op.value;
          target[prop].splice(start, deleteCount, ...items);
        }
        break;
    }
  }
 ...
 * ```
 */
export function stateQL<T extends object>(
 ...
 * @example
 * ```typescript
 * import { createStateQL } from '@in/teract';
 *
