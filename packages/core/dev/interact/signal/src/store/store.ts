...
const proxyTraps: ProxyHandler<InternalStoreNodeType> = {
  get(target, property, receiver) {
    ...
    // 4. Check for properties on the original object using descriptor
  ...
  getOwnPropertyDescriptor(target, property) {
    ...
    if (property === $PROXY)
      return { value: target[$PROXY], writable: true, configurable: true };
    if (property === "isArray" || property === $TARGET_IS_ARRAY)
      return undefined; // Don't expose internal flags
    ...
    if (
      (target as any).isArray ||
      (target as any)[$TARGET_IS_ARRAY] ||
      Array.isArray(target[STORE_VALUE])
    ) {
...
function getAtPath(
  ...
  for (let i = 0; i < path.length - 1; i++) {
    ...
    if (
      ...
      if (__DEV__)
        console.warn(
          `Store path segment '${String(segment)}' not found or invalid.`
        );
      return null;
    }
  ...
  if (current === null || typeof current !== "object") {
    if (__DEV__)
      console.warn(`Store path final parent is not an object/array.`);
    return null;
  }
...
function setProperty(
  ...
  const target = (state as any)[$PROXY]?.[$TARGET] as
    | InternalStoreNodeType
    | undefined;
  if (!target) return;
...
export function createStore<T extends object = {}>(
    ...
    baseSetStore((draft) => {
      ...
      if (targetInfo) {
        ...
        if (Array.isArray(arr)) {
          ...
          console.warn(
            `Store.push: Target at path [${path.join(", ")}] is not an array.`
          );
        }
      }
    });
    ...
    baseSetStore((draft) => {
      ...
      if (targetInfo) {
        ...
        if (Array.isArray(arr)) {
          ...
          console.warn(
            `Store.splice: Target at path [${path.join(", ")}] is not an array.`
          );
        }
      }
    });
    ...
    baseSetStore((draft) => {
      ...
      if (targetInfo) {
        ...
        if (Array.isArray(arr)) {
          ...
          console.warn(
            `Store.remove: Target at path [${path.join(", ")}] is not an array.`
          );
        }
      }
    });
  ...
  if (derived)
    return wrapProjection(first as (store: T) => void, wrappedStore, setStore);

  return [wrappedStore, setStore];
}

// Store the original Array.isArray for internal use
const originalArrayIsArray = Array.isArray;

// Export originalArrayIsArray for use in other modules
export { originalArrayIsArray };

// Create a robust patched version of Array.isArray that works with store proxies
// This function is exported and used in the test-setup.ts file
export function patchedArrayIsArray(obj: any): obj is any[] {
  // First check using built-in Array.isArray (which only works for actual arrays)
  ...
  try {
    // For store proxies, check our marker properties
    if (obj[$TARGET]) {
      // Check if the target has our array marker
      if (obj[$TARGET][$TARGET_IS_ARRAY] === true) return true;
      if ((obj[$TARGET] as any).isArray === true) return true;
    }

    // For raw objects with $RAW, check if the raw value is an array
    if (obj[$RAW] && originalArrayIsArray(obj[$RAW])) return true;

    // Check toString result (this catches most proxy arrays)
...
// Replace Array.isArray with our patched version if not in test environment
// (tests will handle this themselves)
if (typeof globalThis !== "undefined" && !(globalThis as any).ARRAY_PATCHED) {
