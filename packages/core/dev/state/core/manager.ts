...
export class StateManagerClass {
  ...
  private pendingUpdates = new Map<string, Array<(state: any) => Partial<any>>>();
  ...
  public registerState<T extends object>(
    ...
    if (isGlobal) {
      if (this.globalStates.has(id)) {
        console.warn(`Global state with ID '${id}' already exists. Overwriting.`);
      }
    ...
    if (__DEV__) {
      console.debug(`[StateManager] Registered ${isGlobal ? 'global' : 'local'} state: ${id}`);
    }
  }
   ...
   */
  public getGlobalState<T extends object = any>(id: string): StateInstanceType<T> | undefined {
  ...
  public unregisterState(id: string, isGlobal: boolean): void {
    ...
    if (__DEV__) {
      console.debug(`[StateManager] Unregistered ${isGlobal ? 'global' : 'local'} state: ${id}`);
    }
  }
  ...
  public beginBatch(): string {
    ...
    this.currentTransactionId = `batch-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    return this.currentTransactionId;
  }
  ...
  public commitBatch(): void {
    ...
    for (const [stateId, updates] of this.pendingUpdates.entries()) {
      ...
      if (state) {
        ...
        console.warn(`Cannot apply batch updates: State '${stateId}' not found`);
      }
    }
